...
export class ReductionFromEllipsoidComponent extends AbstractCoordinateSystemComponent implements OnInit {
  ...
  private calculate(data) {
    ...
    this.form.controls['calculationName'].setErrors(null);
    const cs: CS = data.cs;
    let geoCS: GeoCS;
    ...
    if ('Line Scale Factor/T-t Correction' === this.calculationName && !(cs instanceof TransverseMercator)) {
      this.calculationError = 'Calculation only valid for UTM Coordinate Systems';
      this.form.controls['calculationName'].setErrors({valid: false});
    }
